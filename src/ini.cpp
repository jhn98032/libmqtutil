 ...
 * Copyright (C) 2014-2026 Johan Henriksson johan[a]dexar.se
 * All rights reserved.
 *
...
#include "mqtutil/ini.h"

#include <QFile>
#include <QStringList>
#include "mqtutil/mqtutil.h"
#include "mqtutil/log.h"
#include <assert.h>
...
int64_t IniEntry::getValueAsInt64() const
{
...
void IniGroup::dump() const
{
    ...
    for(int i = 0;i < m_entries.size();i++)
    {
        ...
        printf("_%s_%s_\n", qPrintable(entry->m_name), qPrintable(valueStr));
    }
...
IniEntry *IniGroup::findEntry(QString entryName)
{
    for(int i = 0;i < m_entries.size();i++)
    {
        ...
        if(entry->m_name == entryName)
            return entry;
        if (entry->m_name.toUpper() == entryName.toUpper())
        {
            errorMsg("%s != %s", qPrintable(entry->m_name),
                     qPrintable(entryName));
        }
...
IniEntry *Ini::findEntry(QString name)
{
    ...
    //assert(entry != NULL);
    //debugMsg("Unable to find '%s'", qPrintable(name));
...
int
Ini::getEntryCount(QString path)
{
    IniGroup *group = findGroup(path);
    if (group)
        return group->m_entries.size();
    return 0;
}
...
void Ini::setInt64(QString name, int64_t value)
{
    IniEntry *entry = addEntry(name, IniEntry::TYPE_INT64);
    entry->m_value = (qlonglong) value;
}
...
void Ini::setStringList(QString name, QStringList valueList)
{
    IniEntry *entry = addEntry(name, IniEntry::TYPE_STRING);
    QString str;
...
int64_t Ini::getInt64(QString name, int64_t defaultValue)
{
    IniEntry *entry = findEntry(name);
    if(!entry)
    {
        setInt64(name, defaultValue);
        entry = findEntry(name);
    }
    return entry->getValueAsInt64();
}
...
{
    QString str = getString(name, defaultValue.join(";"));
    if(str.isEmpty())
        return QStringList();
    QString entry;
    QChar lastC;
...
/**
 * @brief Removes all old entries and loads the content of a ini file.
 * @return 0 on success.
 */
int Ini::load(QString filename)
{
    removeAll();
    return appendLoad(filename);
}



/**
 * @brief Appends the content of a ini file.
 * @return 0 on success.
 */
int Ini::appendLoad(QString filename)
{
    ...
    debugMsg("Ini::%s(filename:\"%s\")", __func__, qPrintable(filename));
            ...
            case STATE_VALUE:
            {
                if(c == QChar('\n') || c == QChar('\r'))
                {
                    ...
                    if(decodeValueString(entry, "", value2.trimmed()))
                        warnMsg("Parse error in %s:L%d", qPrintable(filename), lineNo);
